...
abstract class RoomRemoteRepository<E, X : Any, V> : BaseRoomRemoteRepository<E, X> {
    ...
    open fun getRemoteObj(id: Long, callback: (baseResponse: BaseResponse<E>) -> Unit) {
        getRemoteObj(callback, { api, _, onResponse, onFailure ->
            ...
            insertOrUpdateFromObjApi(id, objApi).subscribe {
                onStored(it)
            }
        })
    }
    ...
    protected fun insertOrUpdateList(offset: Int, apiList: List<X>): Flowable<List<E>> {
        return Flowable.just(apiList)
                .subscribeOn(Schedulers.io())
                .map { apiListRx ->
                    getRoomDataBase().runInTransaction (Callable<List<E>> {
                        insertApiList(offset, apiListRx)
                    })
                }
